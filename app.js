...
/**
 * Renders the Box Plot (Distribution of Case Counts by Location)
 */
function renderBoxPlot(container, rawData, title) {
...
    // Group raw data by location
    const locationData = d3.group(rawData, d => d.location);
    
    // Calculate statistics for each location for the Box Plot
    const boxPlotStats = Array.from(locationData, ([location, values]) => {
        const sortedValues = values.map(d => d.value).sort(d3.ascending);
        const q1 = d3.quantile(sortedValues, 0.25);
        const median = d3.quantile(sortedValues, 0.5);
        const q3 = d3.quantile(sortedValues, 0.75);
        const interQuartileRange = q3 - q1;
        const lowerWhisker = Math.max(d3.min(sortedValues), q1 - 1.5 * interQuartileRange);
        const upperWhisker = Math.min(d3.max(sortedValues), q3 + 1.5 * interQuartileRange);
...
    });
    
    // Sort the box plot data by median (optional but helpful)
    boxPlotStats.sort((a, b) => b.median - a.median);

    // Create the container div with full width
    const chartDiv = container.append('div')
        .attr('class', 'p-4 bg-white rounded-xl shadow-lg m-4 w-full'); // w-full for full screen

    chartDiv.append('h3').attr('class', 'text-lg font-bold text-center mb-1 text-gray-800').text('Distribution of Case Counts by Location');
...
        .range([0, width])
        .domain(boxPlotStats.map(d => d.location))
        .padding(0.6);

    const yMax = d3.max(boxPlotStats, d => d.upperWhisker) * 1.1;
    const y = d3.scaleLinear()
        .range([height, 0])
        .domain([0, yMax]);

    const boxColor = d3.scaleOrdinal(d3.schemeCategory10);
    
    // Axes
    svg.append('g')
...
        .text("Case Count Distribution");

    // Draw the Box Plots
    const boxWidth = x.bandwidth();
    
    const plots = svg.selectAll(".boxPlot")
        .data(boxPlotStats)
        .enter()
        .append("g")
        .attr("transform", d => `translate(${x(d.location)}, 0)`);

    // 1. Vertical Line (Whisker)
    plots.append("line")
        .attr("x1", boxWidth / 2)
        .attr("x2", boxWidth / 2)
...
        .attr("width", boxWidth)
        .attr("stroke", "black")
        .style("fill", d => boxColor(d.location))
        .style("opacity", 0.7);
...
    
    // 4. Outliers (Points)
    plots.selectAll(".outlier")
        .data(d => d.outliers.map(v => ({ location: d.location, value: v })))
        .enter()
        .append("circle")
        .attr("cx", boxWidth / 2)
        .attr("cy", d => y(d.value))
